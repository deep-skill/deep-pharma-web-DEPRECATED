...
const getByIdProductAxios = async (
  id: number,
): Promise<Product | undefined> => {
  try {
    const response = await axios.get(`http://localhost:3001/brand/${id}`);
...
const UpdateProductPage = async ({ params }: { params: { id: number } }) => {
  const productById = await getByIdProductAxios(params.id);
  if (productById === null || productById === undefined)  {
    return (
